$(function ($) {
            ...
            success: function (results) {
                ...
                if (results.success === true) {
                    ...
                    $.each(results.data, function (key, item) {
                        let thisrow = '';
                        thisrow = '<tr class="searchable"><td>'+item['id']+'</td><td>'+item['unit_name']+'</td><td class="text-right"><button data-id='+item['id']+' class="conv btn btn-outline-secondary ml-1"> <i class="fas fa-list"></i></button><button data-id='+item['id']+' class="edit btn btn-outline-secondary ml-1"> <i class="fas fa-cog"></i></button></td></tr>';
    ...
    $('body').on('click', '.edit', function (e) {
        ...
        $.get(route + '/' + $(this).data('id') + '/edit', function (data) {
            ...
            if (data['access']) {
                ...
                $('#edit_name').val(data['record'].unit_name);
    ...
    $('body').on('click', '.conv', function (e) {

        e.preventDefault();
        $('#conv_form').trigger("reset");
        $('#conv_modal').modal('show');
